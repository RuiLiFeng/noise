...
def run(network_pkl, metrics, dataset, data_dir, mirror_augment):
    ...
    tflib.init_tf()
    pkls = [v for v in os.listdir(network_pkl) if v.startswith('network') and v.endswith('.pkl')]
    pkls.sort()
    for pkl in pkls:
        net_pkl = pretrained_networks.get_path_or_url(os.path.join(network_pkl, pkl))
