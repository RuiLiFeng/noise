...
def generate_images(network_pkl, seeds, truncation_psi, data_dir=None, dataset_name=None, model=None):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        misc.save_image_grid(np.exp(np.sum(-x_v, axis=1, keepdims=True)) / np.sum(np.exp(np.sum(-x_v, axis=1, keepdims=True)), axis=(2, 3), keepdims=True),
                             dnnlib.make_run_dir_path('seed%04d-xvs.png' % seed), drange=[0, 1])
        misc.save_image_grid(np.exp(np.sum(-m_v, axis=1, keepdims=True)) / np.sum(np.exp(np.sum(-x_v, axis=1, keepdims=True)), axis=(2, 3), keepdims=True),
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    func_name_map = {
        'generate-images': 'run_generator_visual.generate_images',
