...
def generate_images(network_pkl, seeds, truncation_psi, data_dir=None, dataset_name=None, model=None):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        misc.save_image_grid(adjust_range(n_v[0][0]),
        ...
        misc.save_image_grid(adjust_range(np.sum(np.abs(x_v[0]), axis=0)),
                             dnnlib.make_run_dir_path('seed%04d-xvs.png' % seed), drange=[-1, 1])
        misc.save_image_grid(adjust_range(np.sum(np.abs(m_v[0]), axis=0)),
