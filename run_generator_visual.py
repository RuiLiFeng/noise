...
def generate_images(network_pkl, seeds, truncation_psi, data_dir=None, dataset_name=None, model=None):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        PIL.Image.fromarray(adjust_range(np.sum(np.abs(x_v[0]), axis=0)), 'L').save(
                 dnnlib.make_run_dir_path('seed%04d-xvs.png' % seed))
        PIL.Image.fromarray(adjust_range(np.sum(np.abs(m_v[0]), axis=0)), 'L').save(
            dnnlib.make_run_dir_path('seed%04d-mvs.png' % seed))
