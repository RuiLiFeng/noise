...
def G_main(
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    with tf.control_dependencies(deps):
        images_out, x_v, n_v, m_v = components.synthesis.get_output_for(dlatents, is_training=is_training, force_clean_graph=is_template_graph, **kwargs)

    # Return requested outputs.
    images_out = tf.identity(images_out, name='images_out')
    if return_dlatents:
        return images_out, dlatents
    return images_out, x_v, n_v, m_v
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        if randomize_noise:
            ...
            noise = tf.cast(noise_inputs[layer_idx], x.dtype)
        noise_strength = tf.get_variable('noise_strength', shape=[], initializer=tf.initializers.zeros())
        x_visual = tf.reduce_sum(tf.nn.relu(x), axis=1, name='x_visual')
        noise_visual = tf.reduce_sum(tf.nn.relu(noise), axis=1, name='n_visual')
        x += noise * tf.cast(noise_strength, x.dtype)
        merge_visual = tf.reduce_sum(tf.nn.relu(x), axis=1, name='merge_visual')
    ...
    assert images_out.dtype == tf.as_dtype(dtype)
    return (tf.identity(images_out, name='images_out'),) + visual_array[9]
