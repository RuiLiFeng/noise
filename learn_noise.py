...
def embed(batch_size, resolution, imgs, network, iteration, result_dir, seed=6600):
    ...
    G_args = dnnlib.EasyDict(func_name='training.networks_stylegan2_noise_learn.G_main')
    ...
    noise_pre = [noise_vars[i] +
                 tf.random.normal(name='noise_pre%d' % i,
                                  dtype=tf.float32, shape=noise_vars[i].shape.as_list(), stddev=0.02)
                 for i in range(len(noise_vars))]
    synth_img = G_syn.get_output_for(dlatent, is_training=False, noise_pre=noise_pre, **G_kwargs)
    ...
    for img in imgs:
        ...
        for i in range(iteration):
            loss_, p_loss_, m_loss_, dl_, si_, _ = tflib.run([loss, pcep_loss, mse_loss, dlatent, synth_img, train_op1],
                                                             {img_in: img})
            if i > 3500:
            ...
            if i % 500 == 0:
                si_list.append(si_)
            if i % 100 == 0:
                print('idx %d, Loss %f, mse %f, ppl %f, dl %f, step %d, noise %f' % (idx, loss_, m_loss_, p_loss_, dl_loss_, i, np.mean(noise_pre[0].eval())))
